...
# program to hold the delivery fee and display the total cost
order_program_dict = {
...
order_program_frame = pandas.DataFrame(order_program_dict)
order_program_frame['Total Cost'] = order_program_frame['Total Cost'].apply(currency)
order_program_frame['Delivery Fee'] = order_program_frame['Delivery Fee'].apply(currency)
...
name = input("Start ordering by entering your name: ")
# display menu and calculate burger price
while burger_sold < MAX_BURGERS:
    print("---- Burger Menu ----")
    print()
...
# if users answers "no", set sacue sold to 1 which is the same as the max sauce. so the loop sauce_sold < MAX_SAUCE won't run
elif extra == "no":
    sauce_sold = 1
...
# calculate the sauce price from the list based burger choice
    ...
    print("You chose {}, Burger Price ${:.2f}".format(sauce_choice, sauce_price))
...
payment_method = string_checker("Choose a payment method (cash / "
                                "credit(5%): ",
                                payment_list)
...
# print out the order
# output table with  burger and sacue sold and their price
print("------- Order -------")
print(name)
print("------- Burger -------")
